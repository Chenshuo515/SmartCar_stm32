...
{
    // 更新KEY0状态
    if (HAL_GPIO_ReadPin(Key0.GPIO_Port, Key0.GPIO_Pin) == GPIO_PIN_RESET) {
        // 按键按下
        if (Key0.debounce_counter < 3) {
            Key0.debounce_counter++;
        }

        if (Key0.debounce_counter == 3) {
            if (Key0.state == KEY_RELEASED) {
                Key0.state = KEY_PRESSED;
                Key0.press_start_time = HAL_GetTick();
            } else if (HAL_GetTick() - Key0.press_start_time > 500) {
                Key0.state = KEY_HOLD;
            }
        }
    } else {
        // 按键释放
        if (Key0.debounce_counter > 0) {
            Key0.debounce_counter--;
        }

        if (Key0.debounce_counter == 0) {
            if (Key0.state == KEY_PRESSED || Key0.state == KEY_HOLD) {
                Key0.state = KEY_JUST_RELEASED;
            } else {
                Key0.state = KEY_RELEASED;
            }
        }
...
int8_t Key_GetNum(void)
{
    ...
    if (Key0.state == KEY_PRESSED) {
        keyNum = 0;
        Key0.state = KEY_HOLD; // 防止重复检测
    }
    // 检查KEY0释放事件（可选）
    else if (Key0.state == KEY_JUST_RELEASED) {
        // 可添加释放事件处理
        Key0.state = KEY_RELEASED;
    }

//    // 检查KEY1
//    if (keyNum == -1 && Key1.state == KEY_PRESSED) {
//        keyNum = 1;
